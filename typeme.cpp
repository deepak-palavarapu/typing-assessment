...
void welcomepage()
{
	...
	cout<<"press esc to exist "; i=_getch();
	if(i==13)
		system("CLS");
	else
		exit(1);
...
void level(int pos)
{
	 ...
	 while(1)
	{
	    ...
	    int time=(float(1000)/float((stop_s-start_s)/double(CLOCKS_PER_SEC)*1000))*10000;
     ...
     int reg()
{
	...
	cout<<"press security key to show password and 2 to continue";cin>>ch;
	if(ch==key)
	{
		cout<<pass<<endl;
		system("PAUSE");
...
int main(int argc, char** argv) {
  ...
  cout<<endl;
	...
	cout<<endl<<endl;
 	    cout<<"press enter to start typing or esc to exit"; char ch=_getch();
 	    if(ch==27)
 	    	exit(1);
 	    l:
 	      system("cls");
	 ...
	 cout<<"BYE thanks for using me"<<endl;
   system("PAUSE");
